...
- **AI/LLM:** Claude or GPT-4o via API — structured extraction, analysis, question gen, answer evaluation (all as JSON-schema-constrained calls), called server-side only (never expose the API key to the client)
...
- **File storage:** Cloudflare R2 (resume files + generated TTS audio; S3-compatible, cheap)
...
Per session cost roughly: 1 question-gen LLM call + ~6 TTS calls + ~6 STT calls + ~6 evaluation LLM calls + 1 summary call. Keep an eye on ElevenLabs character limits and Deepgram/Whisper per-minute pricing during testing — cap free sessions per user (the P1 rate limiting) before you demo this publicly so a stress-test doesn't blow through free tiers.
