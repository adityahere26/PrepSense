...
import { ResumeDocumentPreview } from '../components/ResumeDocumentPreview';
...
type DashboardSection = 'overview' | 'resumes' | 'sessions' | 'community';
...
export const Dashboard: React.FC = () => {
  ...
  const {
    ...
    recurringAreas: RecurringImprovementArea[];
    scoreTrendData: any[];
    ...
    queryFn: async () => {
      ...
      const confidence: ConfidenceStats | null = analyticsJson.confidence || null;

      return { pastSessions, recurringAreas, scoreTrendData, confidence };
    },
    enabled: !!token,
  });
  ...
  const confidenceStats = sessionsData?.confidence || null;
  ...
  const navItems: { key: DashboardSection; label: string; icon: React.ElementType }[] = [
    { key: 'overview', label: 'Overview', icon: LayoutGrid },
    { key: 'resumes', label: `Resumes (${resumesList.length})`, icon: FileText },
    { key: 'sessions', label: `Mock Interviews (${pastSessions.length})`, icon: Mic },
    { key: 'community', label: 'Community', icon: MessageSquarePlus },
  ];

  return (
    <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row gap-6 lg:gap-8 items-start">
        {/* SIDEBAR NAVIGATION */}
        <aside className="w-full md:w-60 lg:w-72 shrink-0 md:sticky md:top-24 space-y-4">
          {/* User Summary Card */}
          <Card className="glass-panel p-4 rounded-2xl border-teal-100 bg-white/90 shadow-xs flex items-center gap-3">
            <UserAvatar name={user?.name} email={user?.email} picture={user?.picture} size="md" />
            <div className="min-w-0">
              <p className="text-sm font-bold text-[#043c44] truncate">{user?.name || 'Candidate'}</p>
              <p className="text-[11px] text-slate-500 truncate">{user?.targetRole || 'No target role set'}</p>
            </div>
          </Card>
          ...
          <nav className="flex md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-1 md:pb-0">
            {navItems.map(({ key, label, icon: Icon }) => (
              <button
                ...
                className={`shrink-0 md:w-full flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-colors ${
                  activeSection === key
                    ? 'bg-[#043c44] text-white shadow-xs'
                    : 'bg-white text-slate-600 border border-slate-200 hover:bg-teal-50/60 hover:text-[#043c44]'
                }`}
              >
                <Icon className={`w-4 h-4 shrink-0 ${activeSection === key ? 'text-teal-300' : 'text-[#0d9488]'}`} />
                {label}
              </button>
            ))}
          </nav>
          ...
          <Button
            ...
            className="hidden md:flex w-full px-4 py-2.5 h-auto rounded-xl bg-teal-50 text-[#043c44] border border-teal-200/80 hover:bg-teal-100 font-semibold text-xs items-center justify-center gap-2"
          >
        ...
        <div className="flex-1 min-w-0 space-y-8">
          ...
          {activeSection === 'overview' && (
            <div className="space-y-8">
              ...
              {/* Self-Reported Confidence Delta (pre/post mock interview) */}
              <Card className="glass-panel p-5 sm:p-6 rounded-3xl border-teal-100 bg-white/90 shadow-sm">
                <div className="flex items-center gap-2 mb-1">
                  <Smile className="w-5 h-5 text-[#0d9488]" />
                  <h2 className="font-heading font-bold text-lg text-[#043c44]">Confidence Delta</h2>
                </div>
                <p className="text-xs text-slate-500 mb-4">
                  Self-reported confidence (1-5), captured before and after each mock interview session.
                </p>

                {confidenceStats ? (
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-2xl font-extrabold text-slate-500">{confidenceStats.avgBefore}</p>
                      <p className="text-[11px] text-slate-500">Avg. Before</p>
                    </div>
                    <div>
                      <p className="text-2xl font-extrabold text-[#0d9488]">{confidenceStats.avgAfter}</p>
                      <p className="text-[11px] text-slate-500">Avg. After</p>
                    </div>
                    <div>
                      <p className={`text-2xl font-extrabold ${confidenceStats.avgDelta >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {confidenceStats.avgDelta >= 0 ? '+' : ''}
                        {confidenceStats.avgDelta}
                      </p>
                      <p className="text-[11px] text-slate-500">
                        Avg. Delta ({confidenceStats.sampleSize} session{confidenceStats.sampleSize === 1 ? '' : 's'})
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 text-center">
                    <p className="text-xs text-slate-500">
                      Rate your confidence before and after a mock interview to start tracking this.
                    </p>
                  </div>
                )}
              </Card>
              ...
              <Card className="glass-panel p-5 sm:p-6 rounded-3xl border-teal-100 bg-white/90 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-heading font-bold text-lg text-[#043c44]">Quick Actions</h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button
                    type="button"
                    onClick={() => setActiveSection('resumes')}
                    className="flex items-center gap-3 p-4 rounded-2xl border border-slate-200/80 hover:border-teal-300 hover:bg-teal-50/50 transition-colors text-left"
                  >
                    <div className="w-9 h-9 rounded-xl bg-teal-50 text-[#0d9488] flex items-center justify-center shrink-0">
                      <FileText className="w-4.5 h-4.5" />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-[#043c44]">Review Resumes</p>
                      <p className="text-[11px] text-slate-500">ATS score & rewrite tips</p>
                    </div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#043c44]">Mock Interviews</p>
                      <p className="text-[11px] text-slate-500">Practice & view reports</p>
                    </div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#043c44]">Share Your Story</p>
                      <p className="text-[11px] text-slate-500">Inspire other candidates</p>
                    </div>
                  </button>
                </div>
              </Card>
          ...
          {activeSection === 'resumes' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="font-heading font-bold text-xl text-[#043c44]">Your Resumes</h2>
                <Button
                  size="sm"
                  onClick={handleStartNewResumeUpload}
                  className="text-xs font-semibold rounded-xl bg-teal-50 text-[#043c44] border border-teal-200/80 hover:bg-teal-100"
                >
              ...
              {isLoadingResumes ? (
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <h3 className="font-heading font-bold text-lg text-[#043c44]">All Uploaded Resumes</h3>
                      <p className="text-xs text-slate-500">
                        {resumesList.length} resume group{resumesList.length === 1 ? '' : 's'} total — click any card to view it
                      </p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => setResumesViewMode('split')}
                          ...
                          <div className="flex items-start justify-between gap-3">
                            ...
                            {group.aiQualityScore !== undefined && group.aiQualityScore !== null && (
                              <div className="text-right shrink-0">
                                <span className="text-lg font-extrabold text-[#0d9488]">{group.aiQualityScore}</span>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <h3 className="font-heading font-bold text-base text-[#043c44]">All Resume Groups</h3>
                        <span className="text-xs text-slate-500">{resumesList.length} groups</span>
                      </div>

                      <div className="space-y-3">
                        {resumesList.slice(0, 4).map((group) => {
                          const isActive = activeResumeGroup?.resumeGroupId === group.resumeGroupId;
                        ...
                        {resumesList.length > 4 && (
                          <button
                            type="button"
                            onClick={() => setResumesViewMode('all')}
                          ...
                          >
                            View All ({resumesList.length - 4} more) <ChevronDown className="w-3.5 h-3.5" />
                          </button>
                        )}
                      </div>
                    </div>

                    {/* Right Panel: Selected Resume Header, Document Preview & Quick Actions */}
          ...
          {activeSection === 'sessions' && (
            <div className="space-y-6">
              {isLoadingSessions ? (
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h2 className="font-heading font-bold text-xl text-[#043c44]">Past Voice Interview Sessions</h2>
